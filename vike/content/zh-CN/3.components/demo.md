...
将 `.vue` 文件放到 `demos/` 目录（如 `demos_zh/hello/index.vue`），文件会自动注册——完整路径（含 `.vue` 扩展名）即为 demo 的 key。

例如 `demos_zh/hello/index.vue` 注册为 `hello/index.vue`，然后在 markdown 中引用：
...
```
demos/ 目录 ──→ Vite 插件扫描 ──→ virtual:vvc-demos ──→ contentDemos
sources/ 目录 ──→ Vite 插件扫描 ──→ virtual:vvc-demo-sources ──→ contentSources
